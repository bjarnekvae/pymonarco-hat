...
class Api:
    ...
    def __init__(self, libpath, SPIinterface, SPIclock, name):
        self.__monarco = ctypes.CDLL(libpath)
        self.__cxt = _monarco_cxt_t()
        self.__monarco.monarco_init(ctypes.pointer(self.__cxt), SPIinterface, SPIclock, name)
        self.__cxt.tx_data.led_en = ctypes.c_uint8(0)  # Disable control over LEDs


    def setAO(self, port, value):
        if port == 1:
    ...
    def setPWMfreq(self, channel,  freq):
    ...
    def setPWNout(self, output, DC):
        if output == 1:
            self.__cxt.tx_data.pwm1a_dc = self.__monarco.monarco_util_pwm_dc_to_u16(ctypes.c_double(DC))
        elif output == 2:
            self.__cxt.tx_data.pwm1b_dc = self.__monarco.monarco_util_pwm_dc_to_u16(ctypes.c_double(DC))
        elif output == 3:
            self.__cxt.tx_data.pwm1c_dc = self.__monarco.monarco_util_pwm_dc_to_u16(ctypes.c_double(DC))
        elif output == 4:
            self.__cxt.tx_data.pwm2a_dc = self.__monarco.monarco_util_pwm_dc_to_u16(ctypes.c_double(DC))
